...
Usage:
    ...
    To receive files type receive <<file_name>> while in the client's console to receive a file(must be on the same dir)
    The files will be copied into a folder called received_files at the directory server.py runs at.

    To send files type send <<file_name>>. File to be sent must be in a directory called send that's in the same
